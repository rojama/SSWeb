...
public class Role {
	...
	public Map<String, Object> SaveAuthority (Map<String, Object> map) throws Exception
	{
		...
		String str_Data = trimNull(map.get("TEXT"));
		
		if (!"".equals(str_Data))
		{
			DB.update("delete from aut_role_menu_operation where ROLE_ID='"+ROLE_ID+"' ");
			
			JSONArray lhm_map = JSONArray.parse(str_Data);
			SaveChildrenAuthority(lhm_map,ROLE_ID);
		}
		
		return null;
	}
	
	public void SaveChildrenAuthority(List<Map<String, Object>> list,String ROLE_ID) throws Exception
	{
		for (Map<String, Object> lhm_map : list)
		{
			String Menu_ID=trimNull(((Map<String, Object>) lhm_map.get("data")).get("id"));
			String strSql="insert into aut_role_menu_operation (ROLE_ID,MENU_ID,OPE_ID) values('"+ROLE_ID+"','"+Menu_ID+"','*') ";
			DB.update(strSql);
			
			if(lhm_map.get("children") != null)
			{
				SaveChildrenAuthority((List<Map<String,Object>>)lhm_map.get("children"),ROLE_ID);
			}
		}
	}
