...
public class RoleHelper {
	...
	public List<Map<String, Object>> innerFindInsRole(String Locale,String INS_ID,String INS_SUPER_ID) throws Exception
	{
		...
		if (!"".equals(trimNull(INS_SUPER_ID)))
		{
			...
			strSql.append("and a.ROLE_SUPER_ID is null  order by a.ROLE_ORDER");
		}
		else
		{
			strSql.append("where a.ROLE_SUPER_ID is null  order by a.ROLE_ORDER");
		}
	...
	public  List<Map<String, Object>> innerFindUserRole(String locale,String USER_ID,String ROLE_SUPER_ID) throws Exception
	{
		...
		if (trimNull(ROLE_SUPER_ID).isEmpty()){
			strSql.append("from AUT_ROLE a where a.ROLE_SUPER_ID is null");
		}else{
	...
	public static List<String> findRoleID(String userId)
	{
		List<String> inscode = new ArrayList();		
		try {
			List<Map<String, Object>> insid = DB.query("SELECT c.ROLE_CODE from aut_institution_role a,aut_institution_user b,aut_role c "
					+ "where a.INS_ID=b.INS_ID and c.ROLE_ID=a.ROLE_ID and b.USER_ID='"+userId+"' "
					+ "UNION SELECT f.ROLE_CODE from aut_user_role e,aut_role f "
					+ "where  e.ROLE_ID=f.ROLE_ID and e.USER_ID='"+userId+"'");
			for (Map<String, Object> ins: insid){
				inscode.add((String) ins.get("ROLE_CODE"));
			}
			return inscode;
		} catch (Exception e) {
			e.printStackTrace();
