...
import com.app.help.InstitutionHelper;
import com.app.help.OperationHelper;
import com.app.help.RoleHelper;
import com.app.aut.Role;
...
//				List<Map<String, Object>> data = DB.seleteByKey("aut_user_role", key);			
				if (!data.isEmpty()) {
					for (Map<String, Object> each : data) {
						...
						List<Map<String, Object>> autdata = DB.seleteByColumn("aut_role_menu_operation", key);	
						List<String> permissions = new ArrayList<String>();
						for (Map<String, Object> auteach : autdata){
							permissions.add(auteach.get("MENU_ID")+":"+auteach.get("AUTH_ID"));
						}
	...
	protected AuthenticationInfo doGetAuthenticationInfo(
			...
			if (isAuth)
			{
				...
				subject_session.setAttribute("USER_NAME", InstitutionHelper.findUserName(username));
				subject_session.setAttribute("USER_ID", username);
				subject_session.setAttribute("INS_CODE", InstitutionHelper.findUserIns(username));
				subject_session.setAttribute("ROLE_CODE", RoleHelper.findRoleID(username));
				return info;
			}else{
				return null;
			}
