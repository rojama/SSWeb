...
<%
	...
	String TableName = "TR_COM_FXBOARDSPREAD1";  //数据库表名
...
$(function ()
		{        	
			...
			$grid = $("#maingrid").ligerGrid({
		        ...
		        columns: [  //主表的标题栏位定义
		                    { display: '报价币', name: 'TT_BASECCY', type:"text" },
		        			{ display: '被报价币', name: 'TT_TERMCCY',type:"text"},
		        			
		        			{ display: '期限', name: 'TT_TENOR',type:"text"},
		...
		function toolbarAction(item)
		{
			action = item.icon;
			if (action == "add"){  //新增数据
				$form.setData({TT_BASECCY:'',TT_TERMCCY:'',TT_TENOR:'',TT_BIDPOINT:'',TT_OFFERPOINT:'',TT_MAINTANCEUSER:'',TT_MAINTANCEDATE:''});  
				$form.setData({TT_MAINTANCEUSER:'<%=LoginUserID%>',TT_MAINTANCEDATE:'<%=Systemdate%>'}); 
				$form.setEnabled(['TT_BASECCY','TT_TERMCCY','TT_TENOR','TT_BIDPOINT','TT_OFFERPOINT','TT_MAINTANCEUSER','TT_MAINTANCEDATE'],true);
				...
				$form.setData(select);
				$form.setData({TT_MAINTANCEDATE:'<%=Systemdate%>',TT_MAINTANCEUSER:'<%=LoginUserID%>'}); 
				$form.setEnabled(['TT_BASECCY','TT_TERMCCY','TT_TENOR','TT_BIDPOINT','TT_OFFERPOINT','TT_MAINTANCEUSER','TT_MAINTANCEDATE'],false);
