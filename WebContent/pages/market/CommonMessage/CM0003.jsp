...
<%
	...
	String TableName = "TR_COM_BIZ_FixedHoliday";  //数据库表名
...
$(function ()
		{        	
			...
			$grid = $("#maingrid").ligerGrid({
		        ...
		        columns: [  //主表的标题栏位定义
		        			...
		        			{ display: '月日', name: 'TR_MONTHDAY', minWidth: 80 },
		...
		function toolbarAction(item)
		{
			action = item.icon;
			if (action == "add"){  //新增数据
				$form.setData({TR_CTRYCODE:'',TR_MONTHDAY:'',TR_MAINTAINDATE:'',TR_MAINTAINTIME:'',TR_MAINTAINUSERID:''});  
				$form.setData({TR_MAINTAINDATE:'<%=Systemdate%>',TR_MAINTAINTIME:'<%=Systemtime%>',TR_MAINTAINUSERID:'<%=LoginUserID%>'});  
				$form.setEnabled(['TR_CTRYCODE','TR_MONTHDAY','TR_MAINTAINDATE','TR_MAINTAINTIME','TR_MAINTAINUSERID'],true);
				...
				$form.setData(select);
				$form.setData({TR_MAINTAINDATE:'<%=Systemdate%>',TR_MAINTAINTIME:'<%=Systemtime%>',TR_MAINTAINUSERID:'<%=LoginUserID%>'});  
				$form.setEnabled(['TR_CTRYCODE','TR_MONTHDAY','TR_MAINTAINDATE','TR_MAINTAINTIME','TR_MAINTAINUSERID'],false);
