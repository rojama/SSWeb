...
<%
	...
	String TableName = "TR_COM_BIZ_Weekly";  //数据库表名
...
$(function ()
		{        	
			...
			$grid = $("#maingrid").ligerGrid({
		        ...
		        columns: [  //主表的标题栏位定义
		        			...
		        			{ display: '周休记号', name: 'TR_WEEKHOL', minWidth: 80 },
		...
		function toolbarAction(item)
		{
			action = item.icon;
			if (action == "add"){  //新增数据
				$form.setData({TR_CTRYCODE:'',TR_WEEKHOL:''});  
				$form.setData({MAINTAINDATE:'<%=Systemdate%>', MAINTAINTIME:'<%=Systemtime%>',MAINTAINUSERID:'<%=LoginUserID%>'});
				$form.setEnabled(['TR_CTRYCODE','TR_MAINTAINDATE','MAINTAINDATE','MAINTAINTIME','MAINTAINUSERID'],true);
				...
				$form.setData(select);
				$form.setData({MAINTAINDATE:'<%=Systemdate%>', MAINTAINTIME:'<%=Systemtime%>',MAINTAINUSERID:'<%=LoginUserID%>'});
				$form.setEnabled(['TR_CTRYCODE','TR_MAINTAINDATE','MAINTAINDATE','MAINTAINTIME','MAINTAINUSERID'],false);
